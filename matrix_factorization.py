...
class SVD(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        bu, bi, pu, qi = run_svd(
            ...
            bu=bu,
            global_mean = self.global_mean,
            lr_bi = self.lr_bi if self.lr_bi is not None else self.lr_all,
            lr_bu = self.lr_bu if self.lr_bu is not None else self.lr_all,
            lr_pu = self.lr_pu if self.lr_pu is not None else self.lr_all,
            lr_qi = self.lr_qi if self.lr_qi is not None else self.lr_all,
            n_epochs=self.n_epochs,
            n_factors = self.n_factors,
            pu=pu,
            qi=qi,
            reg_bi = self.reg_bi if self.reg_bi is not None else self.reg_all,
            reg_bu = self.reg_bu if self.reg_bu is not None else self.reg_all,
            reg_pu = self.reg_pu if self.reg_pu is not None else self.reg_all,
            reg_qi = self.reg_qi if self.reg_qi is not None else self.reg_all,
